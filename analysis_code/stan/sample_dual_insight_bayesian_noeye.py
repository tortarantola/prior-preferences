##################################################
### DRIFT DIFFUSION MODEL: BAYESIAN w/BIASED PRIOR & BIASED DRIFT & INSIGHT ###
##################################################
...
# Import data
data = pd.read_pickle(r'../../../data/eye_tracking/fixation_data_merged_wpopularity.pkl')

data2 = data[['participant','img_correct','response_correct','infer_resp_rt','feedback_correct','inf_bid_dv','block_loop_thisN','dwell_pct_pre_resp_correct','dwell_pct_pre_resp_wrong','ratio_prefer_correct']].copy()
...
model_data = {'NS': 31, 'NP': 20, 'NT': 30, 'correct': model_data_correct, 'feedback': model_data_feedback, 'bid_congruence': model_data_bid_congruence, 'rt': model_data_rt, 'rt_mins': model_rt_mins, 'fix_correct_pct': model_data_fix_correct, 'fix_wrong_pct': model_data_fix_wrong, 'fix_neither_pct': model_data_fix_neither, 'item_popularity': model_data_item_popularity}

model_code_obj = pystan.StanModel(file='model_dual_insight_bayesian_noeye.stan.cpp', model_name='model_dual_insight_bayesian_noeye') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('pickles/model_dual_insight_bayesian_noeye.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('pickles/fit_dual_insight_bayesian_noeye.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)
