##################################################
### DRIFT DIFFUSION MODEL: RESCORLA-WAGNER NULL MODEL ###
##################################################
...
model_data = {'NS': 31, 'NP': 20, 'NT': 30, 'correct': model_data_correct, 'feedback': model_data_feedback, 'rt': model_data_rt, 'rt_mins': model_rt_mins, 'fix_correct_pct': model_data_fix_correct, 'fix_wrong_pct': model_data_fix_wrong, 'fix_neither_pct': model_data_fix_neither}

model_code_obj = pystan.StanModel(file='model_null_rw_noeye.stan.cpp', model_name='model_null_rw_noeye') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('pickles/model_null_rw_noeye.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('pickles/fit_null_rw_noeye.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)
