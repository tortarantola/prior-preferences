...
// DRIFT DIFFUSION MODEL: RESCORLA-WAGNER w/BIASED PRIOR (UNBIASED DRIFT PARAMETER)
...
model {
	...
	for (s in 1:NS) {
		...
		for (p in 1:NP) {
			...
			bid_cong[2] <- 0;
			q[2] <- softmax(cong_weight[s] * bid_cong)[1]; // starting point for correct answer
			q[1] <- (1 - q[2]); // starting point for incorrect answer
