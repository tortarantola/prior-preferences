...
model_code_obj = pystan.StanModel(file='model_null_bayesian_noeye.stan.cpp', model_name='model_null_bayesian_noeye') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('pickles/model_null_bayesian_noeye.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('pickles/fit_null_bayesian_noeye.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)
