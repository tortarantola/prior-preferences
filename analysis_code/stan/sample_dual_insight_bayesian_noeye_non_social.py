...
# Import data
data = pd.read_pickle(r'../../../../data/control/data_processed_control_wexclusions_wpopularity.pkl')
...
model_code_obj = pystan.StanModel(file='model_dual_insight_bayesian_noeye_non_social.stan.cpp', model_name='model_dual_insight_bayesian_noeye_non_social') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('pickles/model_dual_insight_bayesian_noeye_non_social.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('pickles/fit_dual_insight_bayesian_noeye_non_social.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)
